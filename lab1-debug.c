...
void queue_print(Queue* q, const char* name)
{
    printf("\tQueue %s: [", name);
    Node* current = q->head;
    while (current) {
...
void* student(void* arg)
{
    ...
    // --- отладочный вывод очереди после добавления ---
    queue_print(&queue_men, "Men");
    queue_print(&queue_women, "Women");
    // -----------------------------------------------
    ...
    // если дождались условия - достаём свой ID из очереди
    queue_pop(student_queue);

    // --- отладочный вывод очереди после извлечения ---
    queue_print(&queue_men, "Men");
    queue_print(&queue_women, "Women");
    // -----------------------------------------------
    ...
    if (bathroom->taken_cabins == 0) {
        ...
        if (waiting_opposite != 0)
        {
            bathroom->current_gender =
                (bathroom->current_gender == MALE) ? FEMALE : MALE;
